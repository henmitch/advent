...
class Floor:
    ...
    def count_accessibles(self) -> int:
        return sum(1 for pos, cell in self
                   if cell == "@" and self.is_accessible(pos))

    def remove_accessibles(self) -> Floor:
        new_grid = []
        for y in range(self.height):
            new_row = ""
            for x in range(self.width):
...
def run(data: Floor) -> int:
    out = 0
    accessible_count = data.count_accessibles()
    while accessible_count > 0:
        out += accessible_count
        data = data.remove_accessibles()
        accessible_count = data.count_accessibles()
    return out
