...
# I could be clever and import and overwrite and stuff, but... I'm lazy.
# For now.
# I'll probably come back and fix this because I'm a nerd.
...
HOME = {
    "A": tuple((3, y) for y in range(2, 6)),
    "B": tuple((5, y) for y in range(2, 6)),
    "C": tuple((7, y) for y in range(2, 6)),
    "D": tuple((9, y) for y in range(2, 6))
}
HOME_POINTS = set()
for points in HOME.values():
    for point in points:
        HOME_POINTS.add(point)
...
def load_data(path: str) -> List[Amphipod]:
    ...
    raw.insert(3, "  #D#C#B#A#")
    raw.insert(4, "  #D#B#A#C#")
    return parse(raw)
...
class Amphipod:
    ...
    def is_done(self) -> bool:
        ...
        if self.loc == self.home[-1]:
            return True
        home_below = self.home[self.home.index(self.loc) + 1:]
        for friend in self.friends:
            if friend.loc in home_below and friend not in self.buddies:
                return False
        return True

    def n_done_buddies(self):
        return sum(buddy.is_done() for buddy in self.buddies)
    ...
    def path(self, endpoint: Point = None) -> Tuple[Point]:
        ...
        if endpoint is None:
            if self.is_done():
                return ((self.loc), )
            endpoint = self.home[len(self.home) - self.n_done_buddies() - 1]
        ...
        while to_review:
            ...
            for adj in adjacent(current) & INBOUNDS:
                ...
                if adj not in so_far or new_cost < so_far[adj]:
                    ...
                    so_far[adj] = new_cost
                    to_review.add(adj)
        raise ValueError(f"No path from {self.loc} to {endpoint}")

    def is_blocked(self) -> bool:
        for friend in self.friends:
            if friend.blocks(self):
                if not (friend in self.buddies and friend.is_done()):
                    pass
                return True
        return False
    ...
    def can_go_home(self) -> bool:
        ...
        #     return True
        if any(f.loc in self.home for f in self.friends - self.buddies):
            return False
        return True
    ...
    def viable_spots(self) -> Set[Point]:
        if self.is_done():
            return set()
        if self.can_go_home():
            if self.n_done_buddies():
                return tuple((self.home[-self.n_done_buddies() - 1], ))
            return ((self.home[-1]), )
        ...
        # go there.
        for friend in self.friends:
            if friend.loc in self.home and friend not in self.buddies:
                spots -= set(self.home)

        if self.loc[1] == 1:
            out = set(self.home) & spots
            if out:
    ...
    def move(self, to: Point) -> bool:
        if to not in self.viable_spots():
            return False
        if to in self.home and any(
                f.blocks(self) for f in self.friends - self.buddies):
            return False
